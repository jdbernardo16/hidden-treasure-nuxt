  ...
  <main>
    ...
    <section class="bg-brand-dark1">
      <div class="max-w-full md:max-w-[1440px] m-auto px-4 md:px-10 pt-20 pb-10">
        <div class="flex items-center justify-between">
          <h2 class="text-lg artegra md:text-4xl text-brand-gold">{{ cms?.frame_2?.header }}</h2>
    ...
    <section class="bg-brand-dark1">
      <div class="max-w-full md:max-w-[1440px] m-auto px-4 md:px-10 py-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        <div v-for="item in cms?.frame_2?.items" :key="item.title" class="flex items-center space-x-6 px-6 py-8 bg-[#535353] text-brand-gold rounded-lg">
          <div class="w-14">
            <nuxt-img :src="item?.icon?.url" alt="icon" width="56" height="56" loading="eager" format="webp" quality="90" />
          </div>
