...
const { data, error } = useFetch(`${API_BASE_URL}/wp-json/wp/v2/package?slug=${route.params.slug}`, {
  query: {
...
<template>
  <Head>
    ...
    <Meta property="og:description" hid="og:description" :content="stripHtml(data?.acf?.content) || ''" />
  ...
  <section class="relative h-[300px] lg:h-[473px]">
    <NuxtImg :src="data?.acf?.image?.url" alt="banner" format="webp" quality="80" loading="eager" class="w-full h-full object-cover" />
    <div class="bg-gradient-to-t from-black to-transparent w-full h-full absolute top-0 left-0 flex flex-col justify-end">
      <div class="px-4 lg:px-10 py-10 lg:py-16 space-y-6">
        <p class="font-bold text-4xl sm:text-6xl artegra text-brand-gold">{{ data?.title?.rendered }}</p>
      </div>
    </div>
  </section>
  <section>
