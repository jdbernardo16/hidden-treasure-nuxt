        ...
        <div class="max-w-[1440px] m-auto lg:px-20 px-4 py-16">
            <div class="grid grid-cols-1 lg:grid-cols-6 gap-10">
                ...
                >
                    ...
                    <div
                        class="text-white space-y-3 mt-4 group-hover/featured-blog:text-brand-gold"
                    >
                        <p
                            class="text-sm text-neutral-100 group-hover/featured-blog:text-brand-gold"
                        >
                            {{ formatDate(blog?.date) }}
                        </p>
                        <p class="text-xl font-bold mb-3 text-brand-gold line-clamp-2">
                            {{ blog?.title?.rendered }}
                        </p>
                ...
                <div class="lg:col-span-3 space-y-10">
                    ...
                    >
                        ...
                        <div
                            class="text-white flex-1 space-y-3 transition group-hover/blog:text-brand-gold"
                        >
                            <p
                                class="text-sm text-neutral-100 transition group-hover/blog:text-brand-gold"
                            >
                                {{ formatDate(blog?.date) }}
                            </p>
                            <p class="text-xl font-bold text-brand-gold line-clamp-2">
                                {{ blog?.title?.rendered }}
                            </p>
                            <p class="line-clamp-2 whitespace-pre-wrap">
                                {{ blog?.acf?.blog_content?.short_description }}
        ...
        <div class="max-w-[1440px] m-auto px-4 lg:px-20 py-10 lg:py-16">
            <div class="flex justify-between items-start">
                <div class="flex space-x-4 items-start mb-10">
                    ...
                    <div>
                        <h2
                            class="text-3xl lg:text-[40px] leading-tight artegra text-brand-gold animateUp"
                        >
                            More Articles
                        </h2>
                    </div>
            ...
            <div class="grid lg:grid-cols-3 gap-10 lg:gap-20 grid-cols-1">
                <nuxt-link
                    :to="`/blog/${blog.slug}`"
                    v-for="blog in nonFeaturedBlogs"
                    class="space-y-4 group/blog cursor-pointer"
                >
...
const featuredBlogs = computed(() =>
    (allBlogs.value || []).filter((blog) => blog.acf.featured_to_homepage),
);
const limit = ref(3);
const nonFeaturedBlogs = computed(() =>
    (allBlogs.value || []).filter((blog) => !blog.acf.featured_to_homepage).slice(0, limit.value),
);
const hasMoreBlogs = computed(
    () =>
        (allBlogs.value || []).filter((blog) => !blog.acf.featured_to_homepage).length >
        limit.value,
);
