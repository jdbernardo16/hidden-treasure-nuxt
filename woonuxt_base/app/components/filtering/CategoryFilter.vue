...
<template>
    <div v-if="terms.length">
        <div
            class="cursor-pointer flex font-semibold mt-8 justify-between items-center text-brand-gold"
            @click="isOpen = !isOpen"
        >
        ...
        <div v-show="isOpen" class="mt-3 mr-1 max-h-[240px] grid gap-1.5">
            <div v-for="term in terms" :key="term.slug" class="flex gap-2 items-start">
                ...
                <label
                    :for="term.slug"
                    class="cursor-pointer m-0 text-sm flex-1 leading-tight !text-white"
                >
