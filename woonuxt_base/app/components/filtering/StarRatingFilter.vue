...
<template>
    <div>
        <div
            class="cursor-pointer flex font-semibold mt-8 leading-none justify-between items-center text-white"
            @click="isOpen = !isOpen"
        >
            <span>{{ $t('messages.shop.rating') }}</span>
            <Icon v-if="isOpen" name="ion:chevron-up-outline" />
            <Icon v-if="!isOpen" name="ion:chevron-down-outline" />
        </div>
        <div v-if="isOpen" class="mt-3 text-sm grid text-gray-500 gap-3">
            ...
            <div class="cursor-pointer flex gap-2 items-center">
                ...
                <label class="flex items-center" for="star-four">
                    ...
                    <span class="ml-1 text-xs text-brand-gold"
                        >& {{ $t('messages.general.up') }}</span
                    >
                </label>
            </div>
            <div class="cursor-pointer flex gap-2 items-center">
