...
const button = ref<{ text: string; color: string }>({ text: t('messages.account.updatePassword'), color: 'bg-brand-gold hover:bg-brand-gold/[0.8]' });
...
const updatePassword = async () => {
  ...
  setTimeout(() => {
    button.value = { text: t('messages.account.updatePassword'), color: 'bg-brand-gold hover:bg-brand-gold/[0.8]' };
  }, 2000);
};
</script>
