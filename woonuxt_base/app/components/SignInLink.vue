...
<template>
    <NuxtLink
        to="/my-account"
        :title="linkTitle"
        class="hidden sm:inline-flex aspect-[1/1] items-center"
    >
        <Transition name="pop-in" mode="out-in">
            <span v-if="avatar" class="relative avatar">
                <User class="w-6 h-6 text-brand-gold" />
                <div class="account-dropdown">
                    <NuxtLink :to="wishlistLink" class="hover:bg-brand-gold hover:text-white"
                        ><Icon name="ion:heart-outline" size="16" /><span>Wishlist</span></NuxtLink
                    >
                    <NuxtLink to="/my-account" class="hover:bg-brand-gold hover:text-white"
