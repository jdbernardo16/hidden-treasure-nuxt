...
<template>
  <div class="flex text-sm leading-none text-gray-400 gap-1 items-center">
    <span class="flex">
      <NuxtLink to="/" class="hover:text-brand-gold line-clamp-1">{{ $t('messages.general.home') }}</NuxtLink>
      <span> /</span>
    </span>
    <span class="flex" v-for="(link, i) in format" :key="link.name || i">
      <NuxtLink v-if="link.slug" :to="decodeURIComponent(link.slug)" class="hover:text-brand-gold line-clamp-1">{{ link.name }}</NuxtLink>
